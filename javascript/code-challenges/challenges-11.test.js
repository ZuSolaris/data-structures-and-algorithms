...
function transformToLis(obj) {
  ...
  // let name = obj.name;
  // console.log(name);
  // let namewrite = document.createElement('li');
  // namewrite.textContent = `name: ${name}`;
  // let age = obj.age;
  // let agewrite = document.createElement('li');
  // agewrite.textContent = `$age: ${age}`;
  // arr.push(namewrite);
  // arr.push(agewrite);
...
const count = (target, input) => {
  //   let counter = 0;
  //   let num = target;
  //   target.forEach((adder) => {
  //     if (num === input) {adder + 1}
  //   });
  let arr = [];
  input.map((i) => {
    i.map((j) => {
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    arr.push(insideArr.filter(x => ((x % 5 === 0) && typeof (x) === 'number')).map(x => Math.pow(2, x)));
